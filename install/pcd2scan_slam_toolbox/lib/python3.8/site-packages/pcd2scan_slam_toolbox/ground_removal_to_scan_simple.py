...
class GroundRemovalScanPublisher(Node):
    ...
    def cloud_cb(self, msg: PointCloud2):
        ...
        if pts.shape[0] == 0:
            ...
            # ☆ 高度去地面：保留高于阈值的点
            # 如果你的雷达坐标系是 z 向上，地面在 z ≈ -1 左右，就用 >
            # 如果你的雷达坐标系地面在 z ≈ 0，就把下面这行改成 pts[:, 2] > 0.1 之类的
        ...
        scan_msg = xy_to_laserscan_msg(
            ...
            angle_increment=self.args.angle_inc,   # 想更快就把默认改成 math.radians(1.0)
            range_min=self.args.range_min,
            range_max=self.args.range_max,
        )
        scan_msg.header.stamp = now_stamp
...
def build_args():
    ...
    # 按高度滤地的阈值：雷达坐标系下 z > 这个值的才算前景
    # 你可以运行一次看看 raw_cloud 的 z 范围，然后调这个
    p.add_argument("--z-thresh", type=float, default=1.0)
